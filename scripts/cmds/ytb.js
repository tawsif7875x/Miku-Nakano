...
async function downloadAndStreamVideo(videoId, videoTitle, videoChannel, message) {
  const apiUrl = `https://www.noobs-api.000.pe/dipto/ytDl3?link=${encodeURIComponent(videoId)}&format=mp4`;

  try {
    const response = await axios.get(apiUrl);

    if (response.data && response.data.downloadLink) {
      const videoURL = response.data.downloadLink;
...
async function downloadAndStreamAudio(videoId, videoTitle, videoChannel, message) {
  const apiUrl = `https://www.noobs-api.000.pe/dipto/ytDl3?link=${encodeURIComponent(videoId)}&format=mp3`;
...
module.exports = {
  ...
  langs: {
    en: {
      ...
      downloading: "⬇ Downloading your %1, please wait.",
      info: "💠 Title: %1\n🏪 Channel: %2\n⏱ Duration: %3\n🔠 ID: %4\n🔗 Link: %5"
    }
  },
